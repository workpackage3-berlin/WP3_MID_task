...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/charlotte/Dropbox/Charite_PhD/tasks/MID_new_version/MID_010724.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    LatePress_blankScreen = keyboard.Keyboard()
    ...
    text_treatCounter = visual.TextStim(win=win, name='text_treatCounter',
        ...
        languageStyle='LTR',
        depth=-2.0);
    LatePress_feedback = keyboard.Keyboard()
    ...
    for thisPracticeLoop in PracticeLoop:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Fixation_training.started', globalClock.getTime())
        ...
        # keep track of which components have finished
        Fixation_trainingComponents = [Fixation2000_fig_training, EarlyPressFixation_training]
        ...
        for thisComponent in Fixation_trainingComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Fixation_training.stopped', globalClock.getTime())
        # check responses
        ...
        # the Routine "Fixation_training" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        if EarlyPressCue_training.keys != None or EarlyPressFixation_training.keys != None and colour == "blue":
            ...
            thisExp.addData('outcome_label_training', "Late")
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('ITI_training.started', globalClock.getTime())
        # keep track of which components have finished
        ITI_trainingComponents = [trial_ITI_training]
        ...
        for thisComponent in ITI_trainingComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('ITI_training.stopped', globalClock.getTime())
    ...
    for thisMainLoop in MainLoop:
        ...
        button_pressed = 0 # Used for pushing button presses
        # keep track of which components have finished
        ...
        # keep track of which components have finished
        FixationComponents = [FixationScreen, EarlyPressFixation]
        ...
        while continueRoutine:
            ...
            if EarlyPressFixation.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                win.callOnFlip(EarlyPressFixation.clearEvents, eventType='keyboard')  # clear events on next screen flip
        ...
        # Calculates target presentation time, based on performance on previous trial.
        if trial_num > 1:
            if prev_trial_acc == "incorrect" or prev_trial_acc == "early" or prev_trial_acc == "late" and resp_time < 1:
                resp_time = resp_time + 0.050
                thisExp.addData('available_resp_time', resp_time);
                thisExp.addData('previous_trial_acc', prev_trial_acc);
            elif prev_trial_acc == "incorrect" or prev_trial_acc == "early" or prev_trial_acc == "late" and resp_time > 1:
                resp_time = resp_time
                thisExp.addData('available_resp_time', resp_time);
                thisExp.addData('previous_trial_acc', prev_trial_acc);
            else:
        ...
        while continueRoutine:
            ...
            if ButtonPressTarget.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                win.callOnFlip(ButtonPressTarget.clearEvents, eventType='keyboard')  # clear events on next screen flip
        ...
        if ButtonPressTarget.keys != None:
            button_pressed = 1
        ...
        if EarlyPressCue.keys != None or EarlyPressFixation.keys != None and colour == "blue":
            ...
            #thisExp.addData('outcome_label', "Late")
            prev_trial_acc = "late"
        ...
        LatePress_blankScreen.keys = []
        LatePress_blankScreen.rt = []
        _LatePress_blankScreen_allKeys = []
        # keep track of which components have finished
        Blank_screenComponents = [LatePress_blankScreen]
        ...
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            # *LatePress_blankScreen* updates
            
            # if LatePress_blankScreen is starting this frame...
            if LatePress_blankScreen.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                LatePress_blankScreen.frameNStart = frameN  # exact frame index
                LatePress_blankScreen.tStart = t  # local t and not account for scr refresh
                LatePress_blankScreen.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(LatePress_blankScreen, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('LatePress_blankScreen.started', t)
                # update status
                LatePress_blankScreen.status = STARTED
                # keyboard checking is just starting
                LatePress_blankScreen.clock.reset()  # now t=0
            
            # if LatePress_blankScreen is stopping this frame...
            if LatePress_blankScreen.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > LatePress_blankScreen.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
                    LatePress_blankScreen.tStop = t  # not accounting for scr refresh
                    LatePress_blankScreen.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('LatePress_blankScreen.stopped', t)
                    # update status
                    LatePress_blankScreen.status = FINISHED
                    LatePress_blankScreen.status = FINISHED
            if LatePress_blankScreen.status == STARTED:
                theseKeys = LatePress_blankScreen.getKeys(keyList=['left','right'], ignoreKeys=["escape"], waitRelease=False)
                _LatePress_blankScreen_allKeys.extend(theseKeys)
                if len(_LatePress_blankScreen_allKeys):
                    LatePress_blankScreen.keys = _LatePress_blankScreen_allKeys[0].name  # just the first key pressed
                    LatePress_blankScreen.rt = _LatePress_blankScreen_allKeys[0].rt
                    LatePress_blankScreen.duration = _LatePress_blankScreen_allKeys[0].duration
        ...
        # check responses
        if LatePress_blankScreen.keys in ['', [], None]:  # No response was made
            LatePress_blankScreen.keys = None
        MainLoop.addData('LatePress_blankScreen.keys',LatePress_blankScreen.keys)

        # we had a response at Blank screen only
        if LatePress_blankScreen.keys != None and EarlyPressCue.keys == None and EarlyPressFixation.keys == None and ButtonPressTarget.keys == None:  
        ...
        LatePress_feedback.keys = []
        LatePress_feedback.rt = []
        _LatePress_feedback_allKeys = []
        # keep track of which components have finished
        FeedbackCodeComponents = [text_Feedback, text_treatCounter, LatePress_feedback]
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # *LatePress_feedback* updates
            waitOnFlip = False
            
            # if LatePress_feedback is starting this frame...
            if LatePress_feedback.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                LatePress_feedback.frameNStart = frameN  # exact frame index
                LatePress_feedback.tStart = t  # local t and not account for scr refresh
                LatePress_feedback.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(LatePress_feedback, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'LatePress_feedback.started')
                # update status
                LatePress_feedback.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(LatePress_feedback.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(LatePress_feedback.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
            # if LatePress_feedback is stopping this frame...
            if LatePress_feedback.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > LatePress_feedback.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    LatePress_feedback.tStop = t  # not accounting for scr refresh
                    LatePress_feedback.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'LatePress_feedback.stopped')
                    # update status
                    LatePress_feedback.status = FINISHED
                    LatePress_feedback.status = FINISHED
            if LatePress_feedback.status == STARTED and not waitOnFlip:
                theseKeys = LatePress_feedback.getKeys(keyList=['left','right'], ignoreKeys=["escape"], waitRelease=False)
                _LatePress_feedback_allKeys.extend(theseKeys)
                if len(_LatePress_feedback_allKeys):
                    LatePress_feedback.keys = _LatePress_feedback_allKeys[0].name  # just the first key pressed
                    LatePress_feedback.rt = _LatePress_feedback_allKeys[0].rt
                    LatePress_feedback.duration = _LatePress_feedback_allKeys[0].duration
        ...
        # check responses
        if LatePress_feedback.keys in ['', [], None]:  # No response was made
            LatePress_feedback.keys = None
        MainLoop.addData('LatePress_feedback.keys',LatePress_feedback.keys)

        # we had a response only at Feedback phase
        if LatePress_feedback.keys != None and LatePress_blankScreen.keys == None and EarlyPressCue.keys == None and EarlyPressFixation.keys == None and ButtonPressTarget.keys == None:  
        ...
        if LatePress_feedback.keys == None and LatePress_blankScreen.keys == None and EarlyPressCue.keys == None and EarlyPressFixation.keys == None and ButtonPressTarget.keys == None and button_pressed == 0:
            ...
            print("Response: [%s]" % resp)    
            outlet.push_sample([resp])  # Push event marker. 
            thisExp.addData('outcome_label', "No response")           
        ...
        # --- Prepare to start Routine "post_break_ITI" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('post_break_ITI.started', globalClock.getTime())
        # Run 'Begin Routine' code from post_break_iti_code
        if MainLoop.thisN != 69:
            continueRoutine = False
        
        # keep track of which components have finished
        post_break_ITIComponents = [post_break_iti_fig]
        for thisComponent in post_break_ITIComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # *post_break_iti_fig* updates
            
            # if post_break_iti_fig is starting this frame...
            if post_break_iti_fig.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                post_break_iti_fig.frameNStart = frameN  # exact frame index
                post_break_iti_fig.tStart = t  # local t and not account for scr refresh
                post_break_iti_fig.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(post_break_iti_fig, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'post_break_iti_fig.started')
                # update status
                post_break_iti_fig.status = STARTED
                post_break_iti_fig.setAutoDraw(True)
            
            # if post_break_iti_fig is active this frame...
            if post_break_iti_fig.status == STARTED:
                # update params
                pass
            
            # if post_break_iti_fig is stopping this frame...
            if post_break_iti_fig.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > post_break_iti_fig.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    post_break_iti_fig.tStop = t  # not accounting for scr refresh
                    post_break_iti_fig.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'post_break_iti_fig.stopped')
                    # update status
                    post_break_iti_fig.status = FINISHED
                    post_break_iti_fig.setAutoDraw(False)
        ...
        # --- Ending Routine "post_break_ITI" ---
        for thisComponent in post_break_ITIComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('post_break_ITI.stopped', globalClock.getTime())
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 23.0 repeats of 'MainLoop'
